...
def read_lammps_data(filepath: str, atom_style: str) -> (dict, dict, tuple):
    ...
    all_sections = {"Masses"} | set(coeff_sections) | topology_sections
    atom_style_columns = {
        "atomic": ["type", "x", "y", "z"],
        "charge": ["type", "q", "x", "y", "z"],
    ...
    while i < n:
        ...
        if section == "Masses":
            ...
            for line in body:
                # drop the leading bond ID -- it's just a sequential counter
    ...
    # --- reduce Atoms lines to (mol_id, type, x, y, z, q), ordered by type ---
    if atoms_body_cache:

        col_names = atom_style_columns.get(atom_style)
        parsed_atoms = []  # (mol_id, type, x, y, z, q)
        for line in atoms_body_cache:
            ...
            if col_names is not None and len(rest) >= len(col_names):
                ...
                # mol id only exists for molecular/full atom styles
                # default to 1 for styles others (atomic/charge)
            ...
            parsed_atoms.append((mol_id, atom_type, x, y, z, q))
...
def write_xyz(atoms, filepath, comment="", element_map=None):
    ...
    :param atoms: Atom data as a list of (mol_id, type, x, y, z, q)
        tuples.
    ...
    with open(filepath, "w") as f:
        f.write(f"{len(atoms)}\n")
        f.write(f"{comment}\n")
        for mol_id, atom_type, x, y, z, q in atoms:
...
def write_lammps_data(construction,
    ...
    xlo, xhi, ylo, yhi, zlo, zhi = box
    atoms = topology["atoms"]  # (mol_id, type, x, y, z, q)
    ...
    with open(filepath, "w") as f:
        ...
        for idx, (mol_id, atype, x, y, z, q) in enumerate(atoms, start=1):
            row = {
                "mol": mol_id,
                "type": atype,
                "q": q,
            ...
            f.write(f"{idx} " + " ".join(str(row[c])
                                         for c in col_names) + "\n")
        f.write("\n")

        for key, label, ncols in [
            ("bonds", "Bonds", 2),
            ("angles", "Angles", 3),
...
def build_combined_system(molecules, atom_style="full"):
    ...
    for m in molecules:
        ...
        for _ in range(n_copies):
            mol_id += 1  # one unique molecule ID per copy
            for (_, atype, x, y, z, q) in mol_topology["atoms"]:
                # discard original mol_id, every copy gets unique mold_id
                combined_topology["atoms"].append(
                    (mol_id, atype + type_offsets["atom"], x, y, z, q))
            for (btype, a1, a2) in mol_topology["bonds"]:
                combined_topology["bonds"].append(
                    (btype + type_offsets["bond"], a1 + atom_id_offset,
                     a2 + atom_id_offset))
            for (antype, a1, a2, a3) in mol_topology["angles"]:
                combined_topology["angles"].append(
                    (antype + type_offsets["angle"], a1 + atom_id_offset,
                     a2 + atom_id_offset, a3 + atom_id_offset))
            for (dtype, a1, a2, a3, a4) in mol_topology["dihedrals"]:
                combined_topology["dihedrals"].append(
                    (dtype + type_offsets["dihedral"], a1 + atom_id_offset,
                     a2 + atom_id_offset, a3 + atom_id_offset,
                     a4 + atom_id_offset))
            for (itype, a1, a2, a3, a4) in mol_topology["impropers"]:
                combined_topology["impropers"].append(
                    (itype + type_offsets["improper"], a1 + atom_id_offset,
                     a2 + atom_id_offset, a3 + atom_id_offset,
                     a4 + atom_id_offset))
...
def substitute_packed_coordinates(topology, packed_atoms):
    ...
    new_atoms = []
    for (mol_id, atype, _, _, _,
         q), (_, x, y, z) in zip(topology["atoms"], packed_atoms):
        new_atoms.append((mol_id, atype, x, y, z, q))
...
def make_molecule_unique(construction,
    ...
    target_indices = [i for i, a in enumerate(atoms) if a[0] == target_mol_id]
    if not target_indices:
        raise ValueError(f"No atoms found with mol_id == {target_mol_id}")

    # types used anywhere OUTSIDE the target molecule
    other_types = {a[1] for a in atoms if a[0] != target_mol_id}
    ...
    # invert to new -> old so callers can iterate new type ids directly
    ...
    for i in target_indices:
        mol_id, old_type, x, y, z, q = new_atoms[i]
        if old_type in old_to_new:
            new_atoms[i] = (mol_id, old_to_new[old_type], x, y, z, q)
    topology["atoms"] = new_atoms
    ...
    return construction, topology, old_to_new
